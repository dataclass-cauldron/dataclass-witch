...
The ``dataclass-witch`` library officially supports **Python 3.9+**
...
* For ``timedelta`` types, the values to de-serialize can either be strings or numbers,
  ...
  module, which is also available as an extra via ``pip install dataclass-witch[timedelta]``.
...
.. _using dataclasses: https://dataclass-witch.readthedocs.io/en/latest/common_use_cases/dataclasses_in_union_types.html
