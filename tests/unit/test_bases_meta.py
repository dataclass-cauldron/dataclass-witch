...
def test_merge_meta_with_and():
    ...
    assert (
        ...
        is merged_meta.marshal_date_time_as
        is A.marshal_date_time_as
    )
...
def test_meta_config_is_not_implicitly_shared_between_dataclasses(mock_log):
    ...
    # Assert all JSON keys are converted to snake case
    expected_json_keys = ["myStr", "listOfInt", "isActive", "myDate", "myDt"]
    assert all(k in d for k in expected_json_keys)
...
def test_meta_initializer_is_called_when_meta_is_an_inner_class(mock_meta_initializers):
    """
...
def test_meta_initializer_errors_when_key_transform_with_load_is_invalid(mock_log):
    """
...
def test_meta_initializer_errors_when_key_transform_with_dump_is_invalid(mock_log):
    """
...
def test_meta_initializer_errors_when_marshal_date_time_as_is_invalid(mock_log):
    """
