...
from dataclass_witch import Container
from dataclass_witch.wizard_mixins import JSONListWizard, JSONFileWizard, YAMLWizard
...
def mock_open(mocker: MockerFixture):
    return mocker.patch("dataclass_witch.wizard_mixins.open")
...
def test_json_file_wizard_methods(mocker: MockerFixture, mock_open):
    ...
    c = MyFileWizard.from_json_file(filename, decoder=mock_decoder)
...
def test_yaml_wizard_methods(mocker: MockerFixture):
    ...
    m = mocker.patch(
        "dataclass_witch.wizard_mixins.open", mocker.mock_open(read_data=yaml_data)
    )
    ...
    assert obj == MyYAMLWizard(
        my_str="test value", inner=Inner(my_float=1.2, my_list=["hello, world!", "123"])
    )
