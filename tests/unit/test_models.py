...
from dataclass_witch import fromlist
from dataclass_witch.models import Container, json_field
from .conftest import SampleClass


@pytest.fixture
def mock_open(mocker: MockerFixture):
    return mocker.patch("dataclass_witch.models.open")
...
def test_container_with_incorrect_usage():
    ...
    assert (
        "A Container object needs to be instantiated with a generic type T" in err_msg
    )


def test_container_methods(mocker: MockerFixture, mock_open):
    list_of_dict = [{"f1": "hello", "f2": 1}, {"f1": "world", "f2": 2}]
